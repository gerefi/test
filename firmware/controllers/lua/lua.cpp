...
void startLua() {
	...
	// we need this on microGerEFI for sure
	// definitely should NOT have this on Proteus
	// on Hellen a bit of open question what's the best track
	// cute hack: let's check at runtime if you are a lucky owner of microGerEFI with extra RAM and use that extra RAM for extra Lua
	if (isStm32F42x()) {
